#!/usr/bin/env node
// QVAC Fake Parking Ticket Excuse Generator — GUI mode.
...
const PORT = process.env.PORT ? Number(process.env.PORT) : 29430;
...
async function main() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url === "/api/ticketexcuse") {
      try {
        ...
        const result = await generate(modelId, reason.trim());
  ...
  server.listen(PORT, () => {
    console.log(`▸ QVAC Fake Parking Ticket Excuse Generator GUI ready at http://localhost:${PORT}`);
  });
